...
public:
    ...
    bool
    addPoint(const void* datapoint, labeltype label, bool replace_deleted = false) {
        int idx;
        {
    ...
    removePoint(labeltype cur_external) {
        ...
        labeltype label =
            *((labeltype*)(data_ + size_per_element_ * (cur_element_count - 1) + data_size_));
    ...
    searchKnn(const void* query_data, size_t k, BaseFilterFunctor* isIdAllowed = nullptr) {
        ...
        dist_t lastdist =
            topResults.empty() ? std::numeric_limits<dist_t>::max() : topResults.top().first;
    ...
    void
    loadIndex(const std::string& location, SpaceInterface* s) {
