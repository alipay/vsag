...
new_error(int type_, const char* msg) {
    ...
    memcpy(err->message,
           msg,
           msg_size > VSAG_MAX_ERROR_MESSAGE_LENGTH ? VSAG_MAX_ERROR_MESSAGE_LENGTH : msg_size);

    return err;
}

void
free_error(const CError* error) {
    if (error) {
...
const CError*
create_index(const char* in_index_type, const char* in_parameters, void** out_index_ptr) {
    ...
    if (!result.has_value()) {
        ...
        return new_error(static_cast<int>(result.error().type), result.error().message.c_str());
    }
...
build_index(void* in_index_ptr,
    ...
    if (!in_index_ptr || !in_ids || !in_vectors || !out_failed_ids || !out_num_failed) {
        return new_error(static_cast<int>(vsag::ErrorType::INVALID_ARGUMENT),
                         "Invalid null argument.");
    }
...
knn_search_index(void* in_index_ptr,
    ...
    if (!in_index_ptr || !in_query_vector || !in_search_parameters || !out_ids || !out_distances ||
        !out_num_results) {
        return new_error(static_cast<int>(vsag::ErrorType::INVALID_ARGUMENT),
                         "Invalid null argument.");
    }
    ...
    query->NumElements(1)->Dim(in_dim)->Float32Vectors(in_query_vector)->Owner(false);
...
load_index(const char* in_file_path,
    ...
    for (uint64_t i = 0; i < num_keys; ++i) {
        ...
        auto file_reader =
            vsag::Factory::CreateLocalFileReader(in_file_path, offsets[i] + sizeof(uint64_t), size);
        rs.Set(keys[i], file_reader);
    }

    std::shared_ptr<vsag::Index> hnsw;
    if (auto index = vsag::Factory::CreateIndex(in_index_type, in_parameters); index.has_value()) {
        hnsw = index.value();
    } else {
