...
public:
    ...
    SetMaxCapacity(uint64_t capacity) override {
        this->max_capacity_ = std::max(capacity, this->total_count_);  // TODO add warning
    }
...
private:
    inline void
    queryImpl(float* resultDists, const float* queryVector, const uint64_t* idx, uint64_t idCount);

    inline void
    queryImpl(float* resultDists,
              std::shared_ptr<Computer<QuantTmpl>> computer,
              const uint64_t* idx,
              uint64_t idCount);

    inline std::shared_ptr<ComputerInterface>
    factoryComputerImpl(const float* query) {
...
template <typename QuantTmpl, typename IOTmpl>
void
FlattenDataCell<QuantTmpl, IOTmpl>::queryImpl(float* resultDists,
