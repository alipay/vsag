...
public:
    ...
    uint32_t
    GetNeighborSize(InnerIdType id) override;

    void
    GetNeighbors(InnerIdType id, Vector<InnerIdType>& neighbor_ids) override;
...
GraphDataCell<IOTmpl, false>::InsertNeighborsById(InnerIdType id,
    ...
    uint32_t neighbor_count = neighbor_ids.size();
    this->io_->Write((uint8_t*)(&neighbor_count), sizeof(neighbor_count), start);
    start += sizeof(neighbor_count);
    this->io_->Write(
        (uint8_t*)(neighbor_ids.data()), neighbor_ids.size() * sizeof(InnerIdType), start);
}

template <typename IOTmpl>
uint32_t
GraphDataCell<IOTmpl, false>::GetNeighborSize(InnerIdType id) {
    auto start = id * this->code_line_size_;
    uint32_t result = 0;
...
template <typename IOTmpl>
void
GraphDataCell<IOTmpl, false>::GetNeighbors(InnerIdType id, Vector<InnerIdType>& neighbor_ids) {
    auto start = id * this->code_line_size_;
