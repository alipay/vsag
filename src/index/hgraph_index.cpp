...
HGraphIndex::add(const DatasetPtr& base) {
    ...
    try {
        ...
        this->basic_flatten_codes_->BatchInsertVector(base->GetFloat32Vectors(),
                                                      base->GetNumElements());
        if (use_reorder_) {
            this->high_precise_codes_->Train(base->GetFloat32Vectors(), base->GetNumElements());
            this->high_precise_codes_->BatchInsertVector(base->GetFloat32Vectors(),
                                                         base->GetNumElements());
        }
        this->hnsw_add(base);
        return failed_ids;
...
HGraphIndex::search_one_graph(const float* query,
    ...
    if (not isIdAllowed || (*isIdAllowed)(ep)) {
    ...
    while (not candidate_set.empty()) {
        ...
        for (auto i = 0; i < count_no_visited; ++i) {
            ...
            if (cur_result.size() < ef || lower_bound > dist) {
                ...
                if (not isIdAllowed || (*isIdAllowed)(to_be_visited[i])) {
...
HGraphIndex::mutually_connect_new_element(InnerIdType cur_c,
    ...
    Vector<InnerIdType> selectedNeighbors(allocator_);
...
HGraphIndex::calcDistanceById(const float* vector, int64_t id) const {
