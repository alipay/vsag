...
void
HGraphIndex::hnsw_build(const DatasetPtr& base) {
    ...
    for (uint64_t i = 0; i < total; ++i) {
...

        {
            ...
            for (auto j = max_level_ - 1; j > level; --j) {
                result = search_one_graph(datas + dim_ * inner_id,
                                          route_graphs_[j],
...
HGraphIndex::search_one_graph(const float* query,
    ...
    auto prefetch_neighbor_visit_num = 3;  // TODO(LHT) Optimize the param;
    ...
    while (not candidate_set.empty()) {
        ...
        if ((-current_node_pair.first) > lower_bound &&
            (cur_result.size() == ef || (!isIdAllowed && !has_deletions))) {
            break;
        }
        candidate_set.pop();
