...
TEST_CASE("build & add", "[ut][hnsw]") {
    ...
    commom_param.allocator_ = std::make_unique<SafeAllocator>(new DefaultAllocator(), true);
...
TEST_CASE("get distance by label", "[ut][hnsw]") {
    ...
    SECTION("static hnsw test") {
        auto* alg_hnsw_static = new hnswlib::StaticHierarchicalNSW(&space, 100, allocator.get());
