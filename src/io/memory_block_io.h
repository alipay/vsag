...
MemoryBlockIO::ReadImpl(uint64_t size, uint64_t offset) const {
    if (checkValidOffset(size + offset)) {
        if (checkInOneBlock(offset, size + offset)) {
            return this->getDataPtr(offset);
        } else {
...
#if defined(ENABLE_SSE)
    _mm_prefetch(getDataPtr(offset), _MM_HINT_T0);  // todo
#endif
}

void
MemoryBlockIO::checkAndRealloc(uint64_t size) {
