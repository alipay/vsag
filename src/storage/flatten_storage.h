...
class FlattenStorage {
public:
    FlattenStorage() = default;

    explicit FlattenStorage(const std::string& initializeJson);  // todo
...
template <typename QuantTmpl, typename IOTmpl>
FlattenStorage<QuantTmpl, IOTmpl>::FlattenStorage(const std::string& initializeJson) {
    // TODO
}

template <typename QuantTmpl, typename IOTmpl>
void
FlattenStorage<QuantTmpl, IOTmpl>::TrainQuantizer(const float* data, uint64_t size) {
    if (this->quantizer_) {
...
template <typename QuantTmpl, typename IOTmpl>
void
FlattenStorage<QuantTmpl, IOTmpl>::InsertVector(const float* vector, uint64_t idx) {
...
void
FlattenStorage<QuantTmpl, IOTmpl>::BatchInsertVector(const float* vectors,
                                                     uint64_t count,
                                                     IDType* idx) {
    if (idx == nullptr) {
...
template <typename IDType>
void
FlattenStorage<QuantTmpl, IOTmpl>::Query(float* resultDists,
                                         const float* queryVector,
                                         const IDType* idx,
                                         uint64_t idCount) {
...
template <typename IDType>
void
FlattenStorage<QuantTmpl, IOTmpl>::BatchQuery(float* resultDists,
                                              const float* queryVector,
                                              const IDType* idx,
                                              uint64_t queryCount,
                                              uint64_t idCount) {
...
template <typename QuantTmpl, typename IOTmpl>
float
FlattenStorage<QuantTmpl, IOTmpl>::ComputePairVectors(uint64_t id1, uint64_t id2) {
...
template <typename QuantTmpl, typename IOTmpl>
const uint8_t*
FlattenStorage<QuantTmpl, IOTmpl>::GetCodesById(uint64_t id) const {
