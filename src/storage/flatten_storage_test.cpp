...
void
TestFlattenStorage(std::unique_ptr<FlattenStorage<QuantT, IOT>>& storage,
                   uint64_t dim,
                   MetricType metric,
                   float error = 1e-5) {
...
TEST_CASE("fp32[ut][flatten_storage]") {
    int dim = 32;
    {
        auto storage = std::make_unique<FlattenStorage<FP32Quantizer<>, MemoryIO>>();
        ...
        TestFlattenStorage(storage, dim, vsag::MetricType::METRIC_TYPE_L2SQR);
    }
    {
        auto storage = std::make_unique<
            FlattenStorage<FP32Quantizer<vsag::MetricType::METRIC_TYPE_IP>, MemoryIO>>();
        storage->SetQuantizer(
            std::make_unique<FP32Quantizer<vsag::MetricType::METRIC_TYPE_IP>>(dim));
        auto alloctor = new DefaultAllocator();
        storage->SetIO(std::make_unique<MemoryIO>(alloctor));
        TestFlattenStorage(storage, dim, vsag::MetricType::METRIC_TYPE_IP);
    }
    {
        auto storage = std::make_unique<
            FlattenStorage<FP32Quantizer<vsag::MetricType::METRIC_TYPE_COSINE>, MemoryIO>>();
        storage->SetQuantizer(
            std::make_unique<FP32Quantizer<vsag::MetricType::METRIC_TYPE_COSINE>>(dim));
        auto alloctor = new DefaultAllocator();
        storage->SetIO(std::make_unique<MemoryIO>(alloctor));
        TestFlattenStorage(storage, dim, vsag::MetricType::METRIC_TYPE_COSINE);
    }
}
...
TEST_CASE("sq8[ut][flatten_storage]") {
    int dim = 32;
    {
        auto storage = std::make_unique<FlattenStorage<SQ8Quantizer<>, MemoryIO>>();
        storage->SetQuantizer(std::make_unique<SQ8Quantizer<>>(dim));
        auto alloctor = new DefaultAllocator();
        storage->SetIO(std::make_unique<MemoryIO>(alloctor));
        TestFlattenStorage(storage, dim, vsag::MetricType::METRIC_TYPE_L2SQR, 0.01);
    }
    {
        auto storage = std::make_unique<
            FlattenStorage<SQ8Quantizer<vsag::MetricType::METRIC_TYPE_IP>, MemoryIO>>();
        storage->SetQuantizer(
            std::make_unique<SQ8Quantizer<vsag::MetricType::METRIC_TYPE_IP>>(dim));
        auto alloctor = new DefaultAllocator();
        storage->SetIO(std::make_unique<MemoryIO>(alloctor));
        TestFlattenStorage(storage, dim, vsag::MetricType::METRIC_TYPE_IP, 0.01);
    }
    {
        auto storage = std::make_unique<
            FlattenStorage<SQ8Quantizer<vsag::MetricType::METRIC_TYPE_COSINE>, MemoryIO>>();
        storage->SetQuantizer(
            std::make_unique<SQ8Quantizer<vsag::MetricType::METRIC_TYPE_COSINE>>(dim));
        auto alloctor = new DefaultAllocator();
        storage->SetIO(std::make_unique<MemoryIO>(alloctor));
        TestFlattenStorage(storage, dim, vsag::MetricType::METRIC_TYPE_COSINE, 0.01);
    }
}