...
// index->add
/////////////////////////////////////////////////////////
TEST_CASE("hnsw float add & search test", "[ft][index][hnsw]") {
    vsag::Options::Instance().logger()->SetLevel(vsag::Logger::Level::kDEBUG);
    ...
    auto metric_type = GENERATE("l2", "ip", "cosine");
    bool need_normalize = metric_type != std::string("cosine");
    std::unordered_map<int64_t, vsag::IndexPtr> used_dims_index;
    const std::string name = "hnsw";
    for (auto i = 0; i < test_dim_count; ++i) {
        srandom(time(nullptr));
        int64_t dim = dims.at(random() % dims.size());
        ...
        auto param = fixtures::generate_hnsw_build_parameters_string(metric_type, dim);
        auto index = vsag::TestFactory(name, param, true);
        vsag::TestAddIndex(index, dim);
        used_dims_index.emplace(dim, index);
    }
    ...
    SECTION("knn search") {
        ...
        for (auto& [dim, index] : used_dims_index) {
            auto dataset_key = fmt::format("[{}][1000][float][classic][{}]", dim, metric_type);
            auto dataset = fixtures::TestPool<fixtures::TestDataset>::Get(dataset_key);
            vsag::TestKnnSearch(index, dataset, search_parameters, top_k, 0.99);
        }
    }
}
